  ...
  return (
    <>
      ...
      <motion.div
        ...
        className={cn(
          "fixed bottom-0 left-0 right-0 z-50 glass-strong border-t",
...
/** Custom seek slider with buffered indicator. */
...
/** Mobile thin progress bar above bar (touch-friendly). */
function MobileSeek({
  currentTime,
  duration,
  buffered,
    ...
    <div className="px-2 pt-1">
      <div className="relative h-5 touch-none">
        <div className="pointer-events-none absolute inset-x-2 top-1/2 h-1 -translate-y-1/2 overflow-hidden rounded-full bg-foreground/10">
          <div className="h-full bg-foreground/15" style={{ width: `${buffered * 100}%` }} />
          <div
            className="-mt-1 h-full bg-foreground/80"
            style={{ width: `${(currentTime / safeMax) * 100}%` }}
          />
        </div>
        <Slider
          value={[currentTime]}
